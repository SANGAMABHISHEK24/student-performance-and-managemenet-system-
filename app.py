...
OPENAI_MODEL = _secret("OPENAI_MODEL", "gpt-4o-mini") # Updated default to faster model
...
# ----------------------------------------------------------------------------
    ...
    page_header("AI Data Assistant", "Ask natural language questions about your classroom's performance.")
    
    if not OPENAI_API_KEY:
        st.warning("⚠️ OpenAI API Key is missing. Please add `OPENAI_API_KEY` to your Streamlit Secrets.")
    else:
        if "messages" not in st.session_state:
            st.session_state.messages = [{"role": "assistant", "content": "How can I help you analyze the student data today?"}]

        for msg in st.session_state.messages:
            st.chat_message(msg["role"]).write(msg["content"])

        if prompt := st.chat_input("E.g., Who are the top 3 students?"):
            st.session_state.messages.append({"role": "user", "content": prompt})
            st.chat_message("user").write(prompt)
            
            # Build lightweight context
            context = f"STUDENT DATA:\n{df_all.to_string(index=False)}\n\nSUBJECTS:\n{fetch_subjects().to_string(index=False)}"
            
            with st.chat_message("assistant"):
                with st.spinner("Analyzing data..."):
                    try:
                        client = OpenAI(api_key=OPENAI_API_KEY)
                        sys_prompt = "You are a helpful education admin assistant. Use the provided database context to answer questions. Keep answers concise, factual, and strictly based on the provided data."
